...
const generateImage = async (url, filename) => {
	const imgPath = `./images/carousel/${filename}.jpg`
	const imageIsExist = await fileIsExist(imgPath)

	if (!imageIsExist) {
        const imgPathFullImage = `./images/${filename}.jpg`
        const imageFullIsExist = await fileIsExist(imgPathFullImage)

        if (imageFullIsExist) {
            await sharp(imgPathFullImage)
                .resize({
                    width: 440,
                    height: 240,
                    fit: sharp.fit.cover,
